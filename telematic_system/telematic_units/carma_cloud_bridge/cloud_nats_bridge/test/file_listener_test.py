...
#Check if TCR is the new message type
def TCR_assert():
    assert (str(event_handler.getNewCarmaCloudMessageType()) == "TCR")

#Check if TCM is the new message type
def TCM_assert():
    assert (str(event_handler.getNewCarmaCloudMessageType()) == "TCM")

if __name__ == '__main__':
    carma_cloud_dir = "/opt/tomcat/logs"
    carma_cloud_logname = "carma_cloud.log"
    log_name = "test.log"
    event_handler = FileListener(carma_cloud_dir, carma_cloud_logname, log_name)
